        ...
        <div className="w-full">
            <form onSubmit={handleSubmit} id="submit" className="flex flex-col mt-5 gap-2">
                ...
                <div className="flex flex-col p-5 max-[610px]:p-0 gap-2">
                    ...
                    <p className="text-xs mb-4">Atenção! Caso você não tenha certeza de qual certidão deve apresentar, <a className="text-blue-500 hover:text-blue-300 transition-all" href={`/${user?.nick}/?aba=chat`}>Clique aqui</a> e abra um ticket para conversar diretamente com um atendente.</p>

                    <div className={`${backCampoClass}`}>
                        <label className={`${titleClass}`}>* Nome Completo do Eleitor:</label>
                        <input
                            required
                            name="nome_completo"
                    ...
                    <div className="flex max-[610px]:flex-col gap-4">
                        <div className={`${backCampoClass} w-1/2 max-[610px]:w-full`}>
                            <label className={`${titleClass}`}>* Número do Título Eleitoral ou CPF</label>
                            <input
                                name="cpf_ou_titulo"
                                className={`${campoClass}`}
                                value={formData.cpf_ou_titulo}
                                onChange={e => setFormData({ ...formData, cpf_ou_titulo: e.target.value })}
                            />
                        </div>
                        <div className={`${backCampoClass} w-1/2 max-[610px]:w-full`}>
                    ...
                    <div>
                        <div className={`${backCampoClass} max-[610px]:w-full w-1/2`}>
                            <label className={`${titleClass}`}>* Opção de filiação (conforme o cadastro na Justiça Eleitoral):</label>
                            <select
                                required
                                name="filiacao"
                                className={`${campoClass}`}
                                value={formData.filiacao}
                                onChange={e => setFormData({ ...formData, filiacao: e.target.value })}
                            >
                                <option value="">Selecione...</option>
                                <option value="mae e pai">NOME DE MÃE E NOME DE PAI</option>
                                <option value="apenas mae">APENAS NOME DE MÃE</option>
                                <option value="apenas pai">APENAS NOME DE PAI</option>
                                <option value="duas maes">NOME DE DUAS MÃES</option>
                                <option value="dois pais">NOME DE DOIS PAIS</option>
                                <option value="duas maes e um pai">NOME DE DUAS MÃES E UM PAI</option>
                                <option value="uma mae e dois pais">NOME DE UMA MÃE E DOIS PAIS</option>
                                <option value="duas maes e dois pais">NOME DE DUAS MÃES E DOIS PAIS</option>
                                <option value="nenhum">NÃO HÁ REGISTRO DE FILIAÇÃO (PAI OU MÃE)</option>
                            </select>
                        </div>

                        <p className="text-xs pl-2 mb-4">Atenção! Preencha apenas os campos de acordo com a filiação marcada acima.</p>

                        <div className="flex flex-col gap-4">
                            {/* LINHA DA MÃE */}
                            {(formData.filiacao.includes('mae') || formData.filiacao === 'mae e pai') && (
                                <div className="flex gap-4 max-[610px]:flex-col">
                                    <div className={`${backCampoClass} w-1/2 max-[610px]:w-full`}>
                                        <label className={`${titleClass}`}>Nome da Mãe:</label>
                                        <input
                                            name="nome_da_mae"
                                            className={`${campoClass}`}
                                    ...
                                    {/* MÃE 2: Só aparece se a opção incluir "duas maes" */}
                                    {formData.filiacao.includes('duas maes') && (
                                        <div className={`${backCampoClass} w-1/2 max-[610px]:w-full`}>
                                            <label className={`${titleClass}`}>Nome da Mãe 2:</label>
                                            <input
                                                name="nome_da_mae2"
                                                className={`${campoClass}`}
                                                value={formData.nome_da_mae2}
                                                onChange={e => setFormData({ ...formData, nome_da_mae2: e.target.value })}
                                            />
                                        </div>
                                    )}
                                </div>
                            ...
                            {/* LINHA DO PAI */}
                            {(formData.filiacao.includes('pai') || formData.filiacao === 'mae e pai') && (
                                <div className="flex gap-4 max-[610px]:flex-col">
                                    <div className={`${backCampoClass} w-1/2 max-[610px]:w-full`}>
                                        <label className={`${titleClass}`}>Nome do Pai:</label>
                                        <input
                                            name="nome_do_pai"
                                            className={`${campoClass}`}
                                    ...
                                    {/* PAI 2: Só aparece se a opção incluir "dois pais" */}
                                    {formData.filiacao.includes('dois pais') && (
                                        <div className={`${backCampoClass} w-1/2 max-[610px]:w-full`}>
                                            <label className={`${titleClass}`}>Nome do Pai 2:</label>
                                            <input
                                                name="nome_do_pai2"
                                                className={`${campoClass}`}
                                                value={formData.nome_do_pai2}
                                                onChange={e => setFormData({ ...formData, nome_do_pai2: e.target.value })}
                                            />
                                        </div>
                                    )}
                                </div>
...
export const VisualizarPedidoAntEleitoral = () => {
